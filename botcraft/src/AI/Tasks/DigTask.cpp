    ...
    Status Dig(BehaviourClient& c, const Position& pos, const bool send_swing, const PlayerDiggingFace face)
    {
        ...
        if (!c.GetCreativeMode())
        {
            ...
            if (is_head_in_fluid)
            {
                ...
                is_head_in_fluid = GetEnchantmentLvl(head_armor.GetNBT(), "minecraft:aqua_affinity") == 0;
            }
            ...
            if (!main_hand.IsEmptySlot())
            {
                ...
                item = AssetsManager::getInstance().GetItem(main_hand.GetBlockID(), main_hand.GetItemDamage());
#else
