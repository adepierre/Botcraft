...
{
    PathfindingTask::PathfindingTask(InterfaceClient& client) :
    ...
    Status PathfindingTask::Process()
    {
        std::shared_ptr<LocalPlayer> local_player = linked_client.entity_manager->GetLocalPlayer();
        ...
        do
        {
            ...
            bool is_goal_loaded;
            {
                std::lock_guard<std::mutex> world_guard(linked_client.world->GetMutex());
                is_goal_loaded = linked_client.world->IsLoaded(goal);
            }

            // Path finding step
            if (!is_goal_loaded)
            {
                std::cout << "[" << linked_client.network_manager->GetMyName() << "] Current goal position " << goal << " is either air or not loaded, trying to get closer to load the chunk" << std::endl;
            ...
            for (int i = 0; i < path.size(); ++i)
            {
                ...
                while (true)
                {
                    ...
                    if (time_count > 1000 * motion_norm_xz / travel_speed)
                    {
                        {
                    ...
                    else
                    {
                        ...
                        Vector3<double> delta_v = motion_vector * delta_t / (1000.0 * motion_norm_xz) * travel_speed;
                        {
                            std::lock_guard<std::mutex> player_lock(local_player->GetMutex());
    ...
    void PathfindingTask::Initialize()
    {
        // Mandatory
        goal = linked_client.blackboard.Get<Position>("pathfinding.goal");

        // Optional
        dist_tolerance = linked_client.blackboard.Get("pathfinding.dist_tolerance", 0);
        min_end_dist = linked_client.blackboard.Get("pathfinding.min_end_dist", 0);
        travel_speed = linked_client.blackboard.Get("pathfinding.travel_speed", 4.317f);
        allow_jump = linked_client.blackboard.Get("pathfinding.allow_jump", true);
    }

    void PathfindingTask::Finalize()
    {
        linked_client.blackboard.Erase("pathfinding.goal");

        linked_client.blackboard.Erase("pathfinding.dist_tolerance");
        linked_client.blackboard.Erase("pathfinding.min_end_dist");
        linked_client.blackboard.Erase("pathfinding.travel_speed");
        linked_client.blackboard.Erase("pathfinding.allow_jump");
    }

    const std::vector<Position> PathfindingTask::FindPath(const Position& start, const Position& end)
    {
        struct PathNode
        {
            Position pos;
        ...
        while (!nodes_to_explore.empty())
        {
            ...
            for (int i = 0; i < neighbour_offsets.size(); ++i)
            {
                ...
                bool is_in_fluid;
                {
                    ...
                    const Block* block = linked_client.world->GetBlock(current_node.pos);
                    is_in_fluid = block && block->GetBlockstate()->IsFluid();

                    // Start with 2 because if 2 is solid, no pathfinding is possible
                    block = linked_client.world->GetBlock(next_location + Position(0, 1, 0));
                    ...
                    block = linked_client.world->GetBlock(current_node.pos + Position(0, 2, 0));
                    surroundings[0] = block && (block->GetBlockstate()->IsSolid() || (is_in_fluid && block->GetBlockstate()->IsFluid()));

                    block = linked_client.world->GetBlock(next_location + Position(0, 2, 0));
                    surroundings[1] = block && (block->GetBlockstate()->IsSolid() || (is_in_fluid && block->GetBlockstate()->IsFluid()));
                    block = linked_client.world->GetBlock(next_location);
                    ...
                    block = linked_client.world->GetBlock(next_location + Position(0, -3, 0));
                    ...
                    {
                        block = linked_client.world->GetBlock(next_next_location + Position(0, 2, 0));
                        surroundings[7] = block && block->GetBlockstate()->IsSolid();
                        block = linked_client.world->GetBlock(next_next_location + Position(0, 1, 0));
                        surroundings[8] = block && block->GetBlockstate()->IsSolid();
                        block = linked_client.world->GetBlock(next_next_location);
                        ...
                        block = linked_client.world->GetBlock(next_next_location + Position(0, -3, 0));
                    ...
                    && !surroundings[6])
                {
                    ...
                    {
                        block = linked_client.world->GetBlock(next_location + Position(0, y, 0));
