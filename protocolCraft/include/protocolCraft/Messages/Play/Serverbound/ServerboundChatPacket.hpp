...
#error "Protocol version not implemented"
        ...
        DECLARE_FIELDS_NAMES(Message);
#elif PROTOCOL_VERSION < 760 /* < 1.19.1 */
        DECLARE_FIELDS_TYPES(std::string, long long int, SaltSignature, bool);
        DECLARE_FIELDS_NAMES(Message,     Timestamp,     SaltSignature, SignedPreview);
#elif PROTOCOL_VERSION < 761 /* < 1.19.3 */
        DECLARE_FIELDS_TYPES(std::string, long long int, long long int, std::vector<unsigned char>, bool,          LastSeenMessagesUpdate);
        DECLARE_FIELDS_NAMES(Message,     Timestamp,     Salt,          Signature,                  SignedPreview, LastSeenMessages);
#else
        DECLARE_FIELDS_TYPES(std::string, long long int, long long int, std::optional<std::array<unsigned char, 256>>, LastSeenMessagesUpdate);
        DECLARE_FIELDS_NAMES(Message,     Timestamp,     Salt,          Signature,                                     LastSeenMessages);
#endif
