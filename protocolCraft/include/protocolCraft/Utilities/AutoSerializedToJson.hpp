    ...
    )
    {
        ...
        else if constexpr (Internal::IsPair<T>)
        {
            ...
            return WrapJsonWithOffsets(Json::Value({
                { "first",  SerializeType<typename T::first_type>(val.first, start_offset != nullptr ? &start_offset->second.first : nullptr, end_offset != nullptr ? &end_offset->second.first : nullptr).value_or(Json::Value()) },
                { "second",  SerializeType<typename T::second_type>(val.second, start_offset != nullptr ? &start_offset->second.second : nullptr, end_offset != nullptr ? &end_offset->second.second : nullptr).value_or(Json::Value()) }
